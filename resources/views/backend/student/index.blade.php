                        ...
                        @foreach ($students as $index => $student)
                            @if(empty(request('search')) || stripos($student->name, request('search')) !== false)
                                <tr>
                                    ...
                                    <td>
                                        @if($student->photo)
                                            <img src="{{ url('backend/' . $student->photo) }}" alt="Foto Siswa" class="img-thumbnail" width="50">
                                        @else
                                    ...
                                    <td>
                                        ...
                                        <form action="{{ route('students.destroy', $student->id) }}" method="POST" style="display:inline-block;" onsubmit="return confirmDelete(event)">
                                            @csrf
                                            @method('DELETE')
